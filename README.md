...
> ⚠️ **重要提醒：关于Easy Chat**
>
> 作者最近在重构、优化项目，但由于复杂度较高，不断出现各种奇怪的 BUG。其中 **Easy Chat（内置简易聊天工具）** 就存在聊天记录加载失败的风险。
>
> **请不要将重要内容存放在Easy Chat。**
>
> ⚠️ **警告：未来版本中，将推翻旧 Easy Chat 的逻辑，届时所有的旧聊天记录都将无法使用。** 
...
该应用使用 Java 开发，绕不开 JVM 这个内存消耗大户。已尽可能地优化了内存使用，JVM 默认的堆内存只有 128MB（未来还会继续改小，作者自用 64MB）。

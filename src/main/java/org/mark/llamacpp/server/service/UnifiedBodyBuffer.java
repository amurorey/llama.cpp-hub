...
/**
 * 统一请求体缓冲：内存 -> 32KB未找到model落盘 -> 流式注入/转发到OutputStream。 替代原 bodyBuffer +
 * deferredOutput，消除多份内存拷贝。
 */
public class UnifiedBodyBuffer {
	...
	public void write(byte[] data) throws IOException {
		...
		synchronized (this) {
			...
			if (!this.spilled && this.allowSpill && this.totalBytes > SPOOL_THRESHOLD) {
				spoolToFile();
				this.spilled = true;
			} else if (!this.spilled && this.memoryBuffer.size() > MEMORY_LIMIT) {
				spoolToFile();
				this.spilled = true;
			}
		}
	...
	/**
	 * 流式转发到目标输出流，在最后一个 } 前注入指定字符串。 从 body 末尾向前扫描找最后一个非字符串内的 }，不依赖写阶段的状态追踪。
	 * 
	...
	/**
	 * 从 body 末尾向前扫描，找到最后一个非字符串内的 } 的位置。 读取尾部最多 1MB 到内存进行扫描，避免加载整个 body。
	 */
